...
export function getWeatherDetails(code: number, isDay: number = 1): WmoCondition {
  const condition = wmoConditions[code] || { desc: 'Variabile', icon: 'cloud', color: 'text-slate-300' };
...
export function getCellColorClass(celsiusValue: number | undefined): string {
  ...
  if (celsiusValue < 10) return 'bg-blue-950/60 text-blue-200 border border-blue-500/20 hover:bg-blue-900/40';
  if (celsiusValue < 20) return 'bg-teal-950/40 text-teal-200 border border-teal-500/20 hover:bg-teal-900/40';
  if (celsiusValue < 30) return 'bg-amber-950/40 text-amber-200 border border-amber-500/20 hover:bg-amber-900/40';
...
export function updateBackgroundTheme(code: number, isDay: number): string {
  ...
  if ([51, 53, 55, 61, 63, 65, 80, 81, 82].includes(code)) return 'from-slate-800 via-sky-900 to-indigo-950';
