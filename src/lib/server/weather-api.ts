...
export async function getOrFetchWeather(
  ...
  const now = Date.now();
  const weatherOk = cached?.weatherData != null && now - (cached.cachedAt?.getTime() ?? 0) < WEATHER_TTL;
  const histOk = cached?.historicalData != null && now - (cached.cachedAt?.getTime() ?? 0) < HISTORICAL_TTL;
