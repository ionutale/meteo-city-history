<script lang="ts">
  ...
  async function fetchWeatherData(lat: number, lon: number, city: string, ctry: string, tz: string) {
    loading = true;
    try {
...
<div class="min-h-screen text-white transition-all duration-700 ease-in-out p-4 md:p-8 bg-gradient-to-br {bgGradient}">
  <div class="max-w-6xl mx-auto">
    ...
    {#if loading && !weatherData}
      <div class="flex flex-col items-center justify-center py-20">
        ...
        <p class="text-indigo-200/70 text-sm animate-pulse">Sincronizzazione dati atmosferici in corso...</p>
      </div>
    {:else if weatherData}
      <main class="grid grid-cols-1 lg:grid-cols-3 gap-6 transition-opacity duration-500">
    ...
    <footer class="mt-12 text-center text-white/30 text-xs border-t border-white/5 pt-6 flex flex-col md:flex-row justify-between items-center gap-4">
      <p>&copy; 2026 MeteoCast. Creato con API Open-Meteo &amp; Geocoding.</p>
      <div class="flex gap-4">
        <a href="https://open-meteo.com" target="_blank" rel="noopener" class="hover:text-white/60 transition-colors">Dati Meteorologici Pubblici</a>
        <span>&bull;</span>
